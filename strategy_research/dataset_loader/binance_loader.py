...
class BinanceLoader:
    ...
    def download(self, timeframe, pair_type, training_pair_list=None):
        ...
        print("training_pair_list: ", training_pair_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", help="Symbol or all")
    parser.add_argument("-t", help="Timeframe (1m, 5m, 15m, 30m, 1h, 2h, 4h, 1d)")
    parser.add_argument(
        "--type", "--type", help="SPOT, UPERP, CPERP"
    )
    args = parser.parse_args()

    binance_loader = BinanceLoader()
    binance_loader.download(args.t, args.type)
