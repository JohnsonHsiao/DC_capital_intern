...
def get_data(coin):
    try:
        pair = f'{coin}USDT'
        df = pd.read_hdf(f'/Vlumes/crypto_data/price_data/binance/1m/{pair}_PERPETUAL.h5')
    except:
...
class Strategy(BackTester):
    ...
    def _strategy(self, df, side='both', **params):
        ...
        bigger_width = int(params['bigger_width'])
        smaller_width = int(params['smaller_width'])
        ...
        # 多單 
        long_entry = (df['width'] > bigger_width) & (df['close'] > df['uband'])
